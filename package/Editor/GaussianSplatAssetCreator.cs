...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        const string kPrefCenterAtOrigin = "nesnausk.GaussianSplatting.CreatorCenterAtOrigin";
        const string kPrefBakeSceneRotation = "nesnausk.GaussianSplatting.CreatorBakeSceneRotation";
        const string kPrefBakeRotationEulerX = "nesnausk.GaussianSplatting.CreatorBakeRotationEulerX";
        const string kPrefBakeRotationEulerY = "nesnausk.GaussianSplatting.CreatorBakeRotationEulerY";
        const string kPrefBakeRotationEulerZ = "nesnausk.GaussianSplatting.CreatorBakeRotationEulerZ";
        const string kPrefBakeMirrorAxis = "nesnausk.GaussianSplatting.CreatorBakeMirrorAxis";
        static readonly Vector3 kDefaultBakeRotationEuler = new(-90.0f, 0.0f, -17.956f);
        ...
        [SerializeField] bool m_CenterAtOrigin = true;
        [SerializeField] bool m_BakeSceneRotation = true;
        [SerializeField] Vector3 m_BakeRotationEuler = kDefaultBakeRotationEuler;
        [SerializeField] MirrorAxis m_BakeMirrorAxis = MirrorAxis.Z;
        ...
        void Awake()
        {
            ...
            m_CenterAtOrigin = EditorPrefs.GetBool(kPrefCenterAtOrigin, true);
            m_BakeSceneRotation = EditorPrefs.GetBool(kPrefBakeSceneRotation, true);
            m_BakeRotationEuler = new Vector3(
                EditorPrefs.GetFloat(kPrefBakeRotationEulerX, kDefaultBakeRotationEuler.x),
                EditorPrefs.GetFloat(kPrefBakeRotationEulerY, kDefaultBakeRotationEuler.y),
                EditorPrefs.GetFloat(kPrefBakeRotationEulerZ, kDefaultBakeRotationEuler.z));
            m_BakeMirrorAxis = (MirrorAxis)EditorPrefs.GetInt(kPrefBakeMirrorAxis, (int)MirrorAxis.Z);
        }
        ...
        void OnGUI()
        {
            ...
            m_CenterAtOrigin = EditorGUILayout.Toggle("Center Bounds At Origin", m_CenterAtOrigin);
            if (EditorGUI.EndChangeCheck())
                EditorPrefs.SetBool(kPrefCenterAtOrigin, m_CenterAtOrigin);
            EditorGUI.BeginChangeCheck();
            m_BakeSceneRotation = EditorGUILayout.Toggle("Bake Scene Rotation", m_BakeSceneRotation);
            if (EditorGUI.EndChangeCheck())
                EditorPrefs.SetBool(kPrefBakeSceneRotation, m_BakeSceneRotation);
            using (new EditorGUI.DisabledScope(!m_BakeSceneRotation))
            {
                EditorGUI.BeginChangeCheck();
                m_BakeRotationEuler = EditorGUILayout.Vector3Field("Bake Rotation", m_BakeRotationEuler);
                if (EditorGUI.EndChangeCheck())
                    SaveBakeRotationEuler();

                GUILayout.BeginHorizontal();
                GUILayout.Space(EditorGUIUtility.labelWidth);
                if (GUILayout.Button("Reset Rotation", GUILayout.Width(120)))
                {
                    m_BakeRotationEuler = kDefaultBakeRotationEuler;
                    SaveBakeRotationEuler();
                }
                GUILayout.EndHorizontal();
            }
            EditorGUI.BeginChangeCheck();
            m_BakeMirrorAxis = (MirrorAxis)EditorGUILayout.EnumPopup("Bake Mirror Axis", m_BakeMirrorAxis);
            if (EditorGUI.EndChangeCheck())
                EditorPrefs.SetInt(kPrefBakeMirrorAxis, (int)m_BakeMirrorAxis);
        ...
        void SaveBakeRotationEuler()
        {
            EditorPrefs.SetFloat(kPrefBakeRotationEulerX, m_BakeRotationEuler.x);
            EditorPrefs.SetFloat(kPrefBakeRotationEulerY, m_BakeRotationEuler.y);
            EditorPrefs.SetFloat(kPrefBakeRotationEulerZ, m_BakeRotationEuler.z);
        }
        ...
        unsafe void CreateAsset()
        {
            ...
            if (m_CenterAtOrigin)
                CenterInputSplatsAtOrigin(inputSplats);
            if (m_BakeMirrorAxis != MirrorAxis.None || m_BakeSceneRotation)
                BakeInputSplatsTransform(inputSplats, m_BakeSceneRotation ? GetBakeRotation() : quaternion.identity, m_BakeMirrorAxis);
        ...
        static unsafe void CenterInputSplatsAtOrigin(NativeArray<InputSplatData> splatData)
        {
            ...
            if (math.lengthsq(center) < 1.0e-10f)
                return;

            var centerJob = new CenterInputSplatsJob
            {
                m_Center = center,
                m_SplatData = splatData
            };
            centerJob.Schedule(splatData.Length, 4096).Complete();
        }

        [BurstCompile]
        struct CenterInputSplatsJob : IJobParallelFor
        {
            ...
            {
                InputSplatData splat = m_SplatData[index];
                splat.pos = (float3)splat.pos - m_Center;
                m_SplatData[index] = splat;
            }
        }

        quaternion GetBakeRotation()
        {
            Quaternion rotation = Quaternion.Euler(m_BakeRotationEuler);
            return math.normalize(new quaternion(rotation.x, rotation.y, rotation.z, rotation.w));
        }

        static void BakeInputSplatsTransform(NativeArray<InputSplatData> splatData, quaternion rotation, MirrorAxis mirrorAxis)
        {
            if (!splatData.IsCreated || splatData.Length == 0)
                return;

            var transformJob = new BakeInputSplatsTransformJob
            {
                m_Rotation = rotation,
                m_MirrorAxis = mirrorAxis,
                m_SplatData = splatData
            };
            transformJob.Schedule(splatData.Length, 4096).Complete();
        }

        [BurstCompile]
        struct BakeInputSplatsTransformJob : IJobParallelFor
        {
            public quaternion m_Rotation;
            public MirrorAxis m_MirrorAxis;
            public NativeArray<InputSplatData> m_SplatData;
            ...
            public void Execute(int index)
            {
                ...
                ApplyMirror(ref pos);
                splat.pos = math.mul(m_Rotation, pos);

                float4 decodedRotation = DecodePackedRotation(new float4(splat.rot.x, splat.rot.y, splat.rot.z, splat.rot.w));
                ApplyMirror(ref decodedRotation);
                quaternion bakedRotation = math.mul(m_Rotation, new quaternion(decodedRotation));
                float4 packedRotation = GaussianUtils.PackSmallest3Rotation(math.normalize(bakedRotation.value));
                splat.rot = new Quaternion(packedRotation.x, packedRotation.y, packedRotation.z, packedRotation.w);

                m_SplatData[index] = splat;
            }

            void ApplyMirror(ref float3 pos)
            {
                switch (m_MirrorAxis)
                {
                    case MirrorAxis.X:
            ...
            void ApplyMirror(ref float4 rotation)
            {
                switch (m_MirrorAxis)
                {
                    case MirrorAxis.X:
                        rotation.yz = -rotation.yz;
                        break;
                    case MirrorAxis.Y:
                        rotation.xz = -rotation.xz;
                        break;
                    case MirrorAxis.Z:
            ...
            static float4 DecodePackedRotation(float4 packedRotation)
            {
                int index = (int)math.round(packedRotation.w * 3.0f);
                float invSqrt2 = math.rsqrt(2.0f);
                float3 xyz = packedRotation.xyz * math.SQRT2 - invSqrt2;
                float w = math.sqrt(math.max(0.0f, 1.0f - math.dot(xyz, xyz)));
                float4 rotation = new float4(xyz, w);

                if (index == 0)
                    return new float4(rotation.w, rotation.x, rotation.y, rotation.z);
                if (index == 1)
                    return new float4(rotation.x, rotation.w, rotation.y, rotation.z);
                if (index == 2)
                    return new float4(rotation.x, rotation.y, rotation.w, rotation.z);
                return rotation;
            }
        }
        ...
        static void ReorderMorton(NativeArray<InputSplatData> splatData, float3 boundsMin, float3 boundsMax)
        {
            ...
            NativeArray<int> newIndexForOld = new(splatData.Length, Allocator.TempJob);
            for (int newIndex = 0; newIndex < order.m_Order.Length; ++newIndex)
                newIndexForOld[order.m_Order[newIndex].Item2] = newIndex;

            ReorderSplatsInPlace(splatData, newIndexForOld);

            newIndexForOld.Dispose();
            order.m_Order.Dispose();
        }

        static void ReorderSplatsInPlace(NativeArray<InputSplatData> splatData, NativeArray<int> newIndexForOld)
        {
            for (int i = 0; i < splatData.Length; ++i)
            {
                int target = newIndexForOld[i];
                while (target != i)
                {
                    InputSplatData tmp = splatData[i];
                    splatData[i] = splatData[target];
                    splatData[target] = tmp;

                    newIndexForOld[i] = newIndexForOld[target];
                    newIndexForOld[target] = target;
                    target = newIndexForOld[i];
                }
            }
        }
